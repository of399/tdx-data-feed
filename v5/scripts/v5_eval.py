"""v5 评估脚本 - 与 v4 对比 30 只核心股
输入: v5 Ollama 模型 + v4 Ollama 模型
输出: 评估报告 + 评分
"""
...
REPORT = Path("/home/jiuben/tdx-data-feed/v5/reports/v5-eval-20260922.md")
...
def main():
    ...
    for code, name in CORE_STOCKS:
        ...
        # v5
        v5_resp, v5_dur, v5_tok = call_ollama("qwen3-14b-tdx-v5", prompt)
        v5_advice = extract_advice(v5_resp)

        # 比较输出差异（关键字）
        v4_words = set(re.findall(r"[\u4e00-\u9fff]{2,}", v4_resp))
        v5_words = set(re.findall(r"[\u4e00-\u9fff]{2,}", v5_resp))
        overlap = len(v4_words & v5_words)
        similarity = overlap / max(len(v4_words | v5_words), 1) * 100
        ...
        same = "✅" if v4_advice == v5_advice else "⚠️"
        print(
            f"  [{len(results):2d}/30] {same} {code} {name[:6]:6} v4={v4_advice} v5={v5_advice} 相似度={similarity:.0f}%"
        )

    # 写报告
    with REPORT.open("w", encoding="utf-8") as f:
        f.write("# v5 评估 vs v4 · 2026-09-22\n\n")
        f.write(f"> 评估时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("> 评估范围: 30 只核心股\n")
        f.write("> 模型对比: qwen3-14b-tdx-fast:14b (v4) vs qwen3-14b-tdx-v5\n\n")

        f.write("## 一、建议一致性\n\n")
        same_count = sum(1 for r in results if r["v4_advice"] == r["v5_advice"])
        f.write(
        ...
        f.write("## 三、详细对比\n\n")
        f.write(
            "| 标的 | v4 建议 | v5 建议 | 相似度 | v4 tok | v5 tok |\n|---|---|---|---|---|---|\n"
        )
        for r in results:
            same = "✅" if r["v4_advice"] == r["v5_advice"] else "⚠️"
            f.write(
                f"| {same} {r['code']} {r['name'][:6]} | {r['v4_advice']} | {r['v5_advice']} | {r['similarity']:.0f}% | {r['v4_tokens']} | {r['v5_tokens']} |\n"
            )
        ...
        f.write("\n## 五、结论\n\n")
        if same_count >= 20:
            f.write("- ✅ **v5 与 v4 高度一致**：v5 训练未破坏原能力\n")
        else:
            f.write(f"- ⚠️ **v5 偏离 v4**：{len(results) - same_count} 只建议不同\n")
        if avg_sim >= 60:
            f.write("- ✅ **输出风格保持**：平均相似度较高\n")
        else:
            f.write(f"- ⚠️ **输出风格变化**：相似度 {avg_sim:.0f}%\n")
        f.write(
            f"- **v5 平均生成**: {sum(r['v5_tokens'] for r in results) / len(results):.0f} tokens\n"
        )
        f.write(
