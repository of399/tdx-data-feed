"""v4-only 推理（独立进程，避免 OOM）
输出 JSONL 到 /tmp/v4_results.jsonl
"""
...
OUT_JSONL = Path("/tmp/v4_results.jsonl")
...
def main():
    print("=== v4-only 推理 ===")
    print(f"  模型: {V4_MERGED}")
    print(f"  输出: {OUT_JSONL}")

    OUT_JSONL.unlink(missing_ok=True)
    f_out = OUT_JSONL.open("w", encoding="utf-8")

    tokenizer = AutoTokenizer.from_pretrained(V4_MERGED, trust_remote_code=True)
    ...
    print("  加载 v4 (int4 nf4)...")
    model = AutoModelForCausalLM.from_pretrained(
        V4_MERGED, quantization_config=bnb, device_map="cuda:1", trust_remote_code=True
    )
    model.eval()
    print("  ✓ 加载完成")
